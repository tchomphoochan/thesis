...
static bool set_conflict(const uint64_t *s1, const uint64_t *s2, int n1, int n2) {
...
static bool has_conflict(const pmhw_txn_t *a, const pmhw_txn_t *b) {
  return set_conflict(a->writeObjIds, b->writeObjIds, a->numWriteObjs, b->numWriteObjs)
    || set_conflict(a->writeObjIds, b->readObjIds, a->numWriteObjs, b->numReadObjs)
    || set_conflict(b->writeObjIds, b->readObjIds, b->numWriteObjs, a->numReadObjs);
}
...
pmhw_retval_t pmhw_schedule(const pmhw_txn_t *txn) {
  ...
  while (1) {
    ...
    if (!queue_full(pending_head, pending_tail, MAX_PENDING)) {
      ...
      pmhw_txn_t *ntxn = &pending_queue[pending_tail].txn;
      qsort(&ntxn->readObjIds, ntxn->numReadObjs, sizeof(uint64_t), &compare_uint64);
      qsort(&ntxn->writeObjIds, ntxn->numWriteObjs, sizeof(uint64_t), &compare_uint64);
