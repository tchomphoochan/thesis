// pmhw_sim_full_bloom.c - Puppetmaster simulation with Bloom filter-based conflict checking
...
static bloom_t read_bloom;
static bloom_t write_bloom;
...
static bool bloom_conflict_check(const pmhw_txn_t *pending) {
  for (int i = 0; i < pending->numReadObjs; ++i)
    if (bloom_query(&write_bloom, pending->readObjIds[i]))
      return true;
  for (int i = 0; i < pending->numWriteObjs; ++i)
    if (bloom_query(&read_bloom, pending->writeObjIds[i]))
      return true;
  for (int i = 0; i < pending->numWriteObjs; ++i)
    if (bloom_query(&write_bloom, pending->writeObjIds[i]))
      return true;
  return false;
}
...
static void rebuild_bloom() {
  bloom_init(&read_bloom);
  bloom_init(&write_bloom);
  for (int i = 0; i < num_active; ++i) {
    const pmhw_txn_t *txn = &active_txns[i];
    for (int j = 0; j < txn->numReadObjs; ++j)
      bloom_insert(&read_bloom, txn->readObjIds[j]);
    for (int j = 0; j < txn->numWriteObjs; ++j)
      bloom_insert(&write_bloom, txn->writeObjIds[j]);
  }
}
          ...
          if (!conflict) {
            ...
            if (assigned_puppet >= 0) {
              ...
              for (int j = 0; j < entry->txn.numReadObjs; ++j)
                bloom_insert(&read_bloom, entry->txn.readObjIds[j]);
              for (int j = 0; j < entry->txn.numWriteObjs; ++j)
                bloom_insert(&write_bloom, entry->txn.writeObjIds[j]);
